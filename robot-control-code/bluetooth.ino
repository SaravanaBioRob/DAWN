...
BLEIntCharacteristic pumpCharacteristic("2A57", BLEWrite);
BLEIntCharacteristic pressureCharacteristic("2A6D", BLENotify); //2A6D UUID for pressure
    ...
    if (central) {
      ...
      while (central.connected()) {
        ...
        if (pumpCharacteristic.written()) {
          ...
          int *message = new int(pumpCharacteristic.value());
